...
def main():
    ...
    for cc in ['lia', 'olia', 'balia', 'wvegas','cubic','reno','pcc']:
        ...
        src.cmd('iperf -c ' + dst.IP() + ' -t 10 -i 0.2 > ./twohost/' + cc + '/host_1_mptcp_' + str(numclients) +'_client_' + str(numservers) + '_server_' + str(numflows) + '_flows.txt &')
        time.sleep(5)
        src2.cmd('iperf -c ' + dst2.IP() + ' -t 10 -i 0.2')
        src.cmd('iperf -c ' + dst.IP() + ' -t 10 -i 0.2 &')
        time.sleep(5)
        src2.cmd('iperf -c ' + dst2.IP() + ' -t 10 -i 0.2 > ./twohost/' + cc + '/host_2_mptcp_' + str(numclients) +'_client_' + str(numservers) + '_server_' + str(numflows) + '_flows.txt')
        net.stop()
